...
class CollaborativeDeepLearning:
    ...
    def pretrain(self, lamda_w=0.1, encoder_noise=0.1, dropout_rate=0.1, activation='relu', batch_size=64, epochs=10):
        '''
        ...
        for input_dim, hidden_dim in zip(self.hidden_layers[:-1], self.hidden_layers[1:]):
            ...
            ae.fit(X_train, X_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_split=0.2 )
    ...
    def finetune(self, train_mat, test_mat, lamda_u=0.1, lamda_v=0.1, lamda_n=0.1, lr=0.001, batch_size=64, epochs=10):
        ...
        #self.cdl_model.summary()
        #self.cdl_model.compile(optimizer='adam', loss=['mse', 'mse'], loss_weights=[1, lamda_n], metrics = ['accuracy'])
        ...
        model_history = self.cdl_model.fit([train_user, train_item, train_item_feat], [train_label, train_item_feat, train_label], epochs=epochs, batch_size=batch_size, validation_split = 0.2)
        return self.cdl_model.get_weights()
    ...
    def matrix2input(self, rating_mat):
        ...
        #print (train_item.shape[0])
        train_item_feat = [self.item_mat[train_item[x]][0] for x in range(train_item.shape[0])]
        #print (len(train_item_feat[0]))
        #print (train_item_feat[0:5])
        #print(train_user[0], train_item[0], train_label[0])
